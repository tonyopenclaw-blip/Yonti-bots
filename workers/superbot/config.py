...
# =============================================================================
# FIRST CROSS STRATEGY — Tony's Insight
# =============================================================================
# INSIGHT: When a price crosses a target level for the first time,
# it usually stays on that side. The first direction of a breakout
# tends to hold in the 15-min window.
#
# Example:
# - YES at $0.40, target/midpoint is $0.50
...
#
# HOW IT WORKS:
# 1. Track the first time price crosses the midpoint ($0.50) in either direction
# 2. Record which direction it crossed first
# 3. After the first cross, bias entries toward that direction:
#    - If crossed UP first → prefer drift_buy entries (long bias)
#    - If crossed DOWN first → prefer drift_short entries (short bias)
# 4. If price is IN the dead zone ($0.45-$0.55) and hasn't crossed yet → WAIT
# 5. Only enter after first confirmed cross with momentum in that direction
#
# IMPLEMENTATION:
# - FirstCrossTracker class tracks per-market crossing state
# - Drift_buy is SKIPPED if price already crossed DOWN first (contradicts momentum)
# - Drift_short is SKIPPED if price already crossed UP first (contradicts momentum)
# - first_cross_direction is recorded in each trade for post-analysis
#
# MIDPOINT = 0.50 (the target level for crossing detection)
# DEAD_ZONE = 0.45-0.55 (no trades until first cross)
# =============================================================================
